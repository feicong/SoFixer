...
private:
	...
	// 不是只看PT_DYNAMIC是否存在，而是至少包含一个非DT_NULL条目。
	// 这用于识别“段在、内容空（仅DT_NULL）”的转储场景。
	// allow_missing_terminator控制“缺少DT_NULL终止符”时是否按兼容模式继续：
	// 1）true：保持兼容，允许继续解析；
	// 2）false：按不可用处理，触发baseso回填路径。
	bool HasUsableLoadedDynamicSection(bool allow_missing_terminator);
