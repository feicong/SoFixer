...
#ifndef PAGE_SIZE
// 默认按4KB页处理地址对齐。
#define PAGE_SIZE static_cast<Elf_Addr>(0x1000)
#endif

#ifndef PAGE_MASK
// 页掩码
#define PAGE_MASK (~(static_cast<Elf_Addr>(PAGE_SIZE) - static_cast<Elf_Addr>(1)))
#endif

#ifndef PAGE_START
// 取地址x所在页的起始地址。
#define PAGE_START(x) (static_cast<Elf_Addr>((x) & PAGE_MASK))
#endif

#ifndef PAGE_OFFSET
// 取地址x在页内的偏移。
#define PAGE_OFFSET(x) (static_cast<Elf_Addr>((x) & ~PAGE_MASK))
#endif

#ifndef PAGE_END
// 取包含地址x的末尾页边界（向上按页对齐）。
#define PAGE_END(x) PAGE_START(static_cast<Elf_Addr>((x) + (PAGE_SIZE - static_cast<Elf_Addr>(1))))
#endif
...
#define FLOGE(fmt, ...) \
	sofixer::logger::log(sofixer::log_level::error, __FUNCTION__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__)
#define FLOGW(fmt, ...) \
	sofixer::logger::log(sofixer::log_level::warning, __FUNCTION__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__)
#define FLOGI(fmt, ...) \
	sofixer::logger::log(sofixer::log_level::info, __FUNCTION__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__)
#define FLOGD(fmt, ...) \
	sofixer::logger::log(sofixer::log_level::debug, __FUNCTION__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__)
#define FLOGV(fmt, ...) \
	sofixer::logger::log(sofixer::log_level::verbose, __FUNCTION__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__)
#endif	// ANDDBG_ALOG_H
